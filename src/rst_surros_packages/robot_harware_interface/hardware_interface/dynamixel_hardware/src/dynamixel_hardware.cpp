  ...
  const rclcpp::Duration & period )
{
  if (use_dummy_) {
    ...
    double vel_cmd = std::isfinite(joint.command.velocity) ? joint.command.velocity : 0.0;
    double pos_cmd = std::isfinite(joint.command.position) ? joint.command.position : joint.state.position;

    switch (control_mode_) {
      case ControlMode::Velocity:
        joint.state.position += vel_cmd * period.seconds();
        joint.state.velocity  = vel_cmd;
        break;

      case ControlMode::Position:
        joint.state.position = pos_cmd;
        joint.state.velocity = 0.0;
        break;

      default:
        // Hold last state
        joint.state.position = joint.state.position;
        joint.state.velocity = 0.0;
        break;
    }
  }
  ...
  const rclcpp::Duration & period)
{
  if (use_dummy_) {
    ...
    for (auto & joint : joints_) {
      if (std::isfinite(joint.command.velocity) && std::abs(joint.command.velocity) > 1e-6) {
        vel_active = true;
      }
